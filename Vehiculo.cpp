...
bool Vehiculo::quitarDecorador(Item* decorador) {
    string id = decorador->getId();

    if (listaDecoradores->buscarId(id) != nullptr) {
        if (id == "M1" || id == "M2") {
            listaDecoradores->eliminarNodoPorId(id);
            this->setPrecio(this->precio - decorador->getPrecio());
            this->setPotencia(this->potencia - decorador->getPotencia());
            return true;
        } else {
            if (id == "LL1" || id == "LL2" || id == "LL3") {
                listaDecoradores->eliminarNodoPorId(id);
                this->setPrecio(this->precio - decorador->getPrecio());
                this->setTraccion(this->traccion - decorador->getTraccion());
                return true;
